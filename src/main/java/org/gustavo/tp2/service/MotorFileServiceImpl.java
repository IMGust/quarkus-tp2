...
@ApplicationScoped
public class MotorFileServiceImpl implements FileService {
    ...
    public void salvar(Long id, FileUpload file) throws IOException {
        ...
        if (motor == null) {
            throw new NotFoundException("Motor não encontrado.");
        }
    ...
    public void remover(String fid) {
        ...
        // Remove association from Motor
        // Assuming Arquivo is joined with Motor via JoinTable in Motor entity
        // We need to find which motor has this image to remove it from the list
        // Alternatively, since it's @OneToMany with orphanRemoval, deleting from repo might work if correctly mapped.
        // But the professor's code does: planoRepository.findByArquivoId(arquivo.getId()).ifPresent(plano -> plano.removeArquivo(arquivo));
        
        // I need a way to find Motor by Arquivo ID if I want to replicate exactly.
        // Let's check if MotorRepository has findByArquivoId. Probably not.
        // I'll add a method to MotorRepository if needed or just use a query.
        
        motorRepository.find("SELECT m FROM Motor m JOIN m.imagens i WHERE i.id = ?1", arquivo.getId())
                .firstResultOptional()
                .ifPresent(motor -> motor.removeImagem(arquivo));
