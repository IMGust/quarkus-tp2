...
@ApplicationScoped
public class SobrealimentacaoFileServiceImpl implements SobrealimentacaoFileService, TurboFileService, SuperChargerFileService {
    ...
    public void salvar(Long id, FileUpload file) throws IOException {
        Sobrealimentacao s = sobrealimentacaoRepository.findById(id);
        if (s == null) {
            throw new NotFoundException("Sobrealimentação não encontrada.");
        }
    ...
    public void remover(String fid) {
        ...
        sobrealimentacaoRepository.find("SELECT s FROM Sobrealimentacao s JOIN s.imagens i WHERE i.id = ?1", arquivo.getId())
                .firstResultOptional()
                .ifPresent(s -> s.removeImagem(arquivo));
