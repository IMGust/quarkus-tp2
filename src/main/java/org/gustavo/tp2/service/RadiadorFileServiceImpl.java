...
@ApplicationScoped
public class RadiadorFileServiceImpl implements RadiadorFileService {
    ...
    public void salvar(Long id, FileUpload file) throws IOException {
        Radiador radiador = radiadorRepository.findById(id);
        if (radiador == null) {
            throw new NotFoundException("Radiador não encontrado.");
        }
    ...
    public void remover(String fid) {
        ...
        radiadorRepository.find("SELECT r FROM Radiador r JOIN r.imagens i WHERE i.id = ?1", arquivo.getId())
                .firstResultOptional()
                .ifPresent(radiador -> radiador.removeImagem(arquivo));
