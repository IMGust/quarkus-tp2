...
@ApplicationScoped
public class PistaoFileServiceImpl implements PistaoFileService {
    ...
    public void salvar(Long id, FileUpload file) throws IOException {
        Pistao pistao = pistaoRepository.findById(id);
        if (pistao == null) {
            throw new NotFoundException("Pistão não encontrado.");
        }
    ...
    public void remover(String fid) {
        ...
        pistaoRepository.find("SELECT p FROM Pistao p JOIN p.imagens i WHERE i.id = ?1", arquivo.getId())
                .firstResultOptional()
                .ifPresent(pistao -> pistao.removeImagem(arquivo));
