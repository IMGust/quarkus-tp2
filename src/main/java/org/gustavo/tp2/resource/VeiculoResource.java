...
import org.gustavo.tp2.service.VeiculoFileService;
...
public class VeiculoResource {
    ...
    @Inject
    VeiculoFileService fileService;
    ...
    public Response salvarImagem(
            @RestForm("idVeiculo") 
            @NotNull(message = "idVeiculo é obrigatório.")
            @Min(value = 1, message = "idVeiculo deve ser maior ou igual a 1.")
            Long idVeiculo,
        ...
        try {
            fileService.salvar(idVeiculo, file);
            return Response.noContent().build();
        } catch (IOException e) {
